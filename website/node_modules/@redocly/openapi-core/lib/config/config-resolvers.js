...
import { getCachedPlugins, hasCachedPlugin, loadPluginModule, setCachedPlugins, } from './plugins-cache.js';
    ...
    const requireFunc = async (plugin) => {
        ...
        try {
            ...
            if (!hasCachedPlugin(absolutePluginPath)) {
                ...
                if (pluginModule) {
                    setCachedPlugins(absolutePluginPath, pluginInstances.map((p) => ({
                        ...p,
                        path: plugin,
        ...
        return (await Promise.all(pluginInstances.map(async (pluginInstance) => {
            ...
            if (pluginInstance.rules) {
                if (!pluginInstance.rules.oas3 &&
                    ...
                    throw new Error(`Plugin rules must have \`oas3\`, \`oas2\`, \`async2\`, \`async3\`, \`arazzo\`, \`overlay1\`, or \`openrpc1\` rules "${p}.`);
                }
                plugin.rules = {};
                if (pluginInstance.rules.oas3) {
            ...
            if (pluginInstance.preprocessors) {
                if (!pluginInstance.preprocessors.oas3 &&
                    ...
                    throw new Error(`Plugin \`preprocessors\` must have \`oas3\`, \`oas2\`, \`async2\`, \`async3\`, \`arazzo1\`, \`overlay1\`, or \`openrpc1\` preprocessors "${p}.`);
                }
                ...
                if (pluginInstance.preprocessors.openrpc1) {
                    plugin.preprocessors.openrpc1 = prefixRules(pluginInstance.preprocessors.openrpc1, id);
                }
            }
            if (pluginInstance.decorators) {
                if (!pluginInstance.decorators.oas3 &&
                    ...
                    throw new Error(`Plugin \`decorators\` must have \`oas3\`, \`oas2\`, \`async2\`, \`async3\`, \`arazzo1\`, \`overlay1\`, or \`openrpc1\` decorators "${p}.`);
                }
                plugin.decorators = {};
                if (pluginInstance.decorators.oas3) {
