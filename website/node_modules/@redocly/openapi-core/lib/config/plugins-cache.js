...
function registerEsmCacheBustHook() {
    module.registerHooks({
        ...
        resolve(specifier, context, nextResolve) {
            ...
            if (resolvedURL.pathname.includes('/node_modules/'))
                return result;
            ...
            if (resolvedURL.searchParams.has(PLUGIN_VERSION_PARAM))
                return result;
...
function ensureEsmCacheBustHook() {
    if (isEsmCacheBustHookRegistered)
        return;
    isEsmCacheBustHookRegistered = true;
    if (typeof module.registerHooks !== 'function') {
        logger.warn(`Redocly plugin reload requires Node.js >= 22.15 (current: ${process.version}). ` +
            `ESM plugins will not pick up edits until the process restarts.\n`);
        return;
    }
    registerEsmCacheBustHook();
}
export function hasCachedPlugin(absolutePluginPath) {
    return pluginsCache.has(absolutePluginPath);
}
export function getCachedPlugins(absolutePluginPath) {
    return pluginsCache.get(absolutePluginPath);
}
export function setCachedPlugins(absolutePluginPath, plugins) {
