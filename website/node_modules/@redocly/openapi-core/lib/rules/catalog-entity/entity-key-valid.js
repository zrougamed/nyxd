...
export const EntityKeyValid = () => {
    return {
        any(node, { report, location }) {
            if (isPlainObject(node) && 'key' in node) {
                ...
                if (!validKeyPattern.test(key)) {
                    report({
                        message: 'Entity `key` must contain only lowercase letters, numbers, and hyphens, and cannot start or end with a hyphen.',
                        location: location.child(['key']),
                    });
                }
            }
...
//# sourceMappingURL=entity-key-valid.js.map